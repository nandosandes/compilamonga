...
public class PrintVisitor implements Visitor, Constantes {
	...
	private void encerrarEscopo() {
		...
		while (!topoDaPilha.equals("$$$")) {
			...
			tabelaDeSimbolos.remove(topoDaPilha);
	...
	public void visit(ExpressaoBinaria visit) throws Exception {
		...
		if (visit.getOperadorString().equals("+")) {
			if (visit.getExp_esq().getFlag() == INT) {
				if (visit.getExp_dir().getFlag() == INT) {
					...
					visit.setParentValorInt((int) (visit.getExp_esq()
							.getValorInt() + visit.getExp_dir().getValorInt()));
					...
					visit.setFlag(FLOAT);
					visit.setValParentFloat(((float) visit.getExp_esq()
							.getValorInt()) + visit.getExp_dir().getValFloat());
				...
				visit.setFlag(FLOAT);
				visit.setValParentFloat(auxf + visit.getExp_esq().getValFloat());
			}
		...
		// visit.setParentValorInt((int) (visit.getExp_esq()
		// .getValorInt() + visit.getExp_dir().getValorInt()));
		// System.out.println("passou no switch soma int "+" soma int "+(auxIntDir+auxIntEsq));
		// }
		// break;
		// case FLOAT:
		// visit.setFlag(FLOAT);
		// System.out.println("entrou no float "+"soma float "+(auxFloatDir +
		// auxFloatEsq));
		// visit.setValParentFloat((auxFloatDir + auxFloatEsq));
		// break;
		// default:
		// break;
		// }
		...
		// case INT:
		// visit.setFlag(INT);
		// visit.setParentValorInt((auxIntDir*auxIntEsq));
		// break;
		// case FLOAT:
		// visit.setFlag(FLOAT);
		// visit.setValParentFloat((auxFloatDir*auxFloatEsq));
	...
	public void visit(ComandoAtribuicao comando) throws Exception {
		...
		// analisa se o tipo da variavel é igual ao tipo do valor
		if (var.tipo == INT) {
		...
		System.out.println("tttt " + comando.getExp().getValorInt());
	}
